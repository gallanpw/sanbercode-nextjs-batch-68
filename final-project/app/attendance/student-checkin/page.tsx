  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/attendance/checkin', {
        ...
        body: JSON.stringify({ studentIdNumber: nisn }),
      });
      ...
      if (response.ok) {
        ...
        setNisn(''); // Kosongkan input setelah sukses
        // Opsional: Redirect atau berikan opsi lain setelah sukses
        // setTimeout(() => router.push('/'), 3000); // Kembali ke home setelah 3 detik
      } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 flex flex-col items-center justify-center p-4">
      <Card className="w-full max-w-md bg-white dark:bg-gray-800 shadow-xl rounded-2xl overflow-hidden">
        <CardHeader className="text-center p-6 bg-blue-600 dark:bg-indigo-700 text-white">
          <CardTitle className="text-3xl font-extrabold mb-2">Absensi Siswa</CardTitle>
          <CardDescription className="text-blue-100 dark:text-indigo-200">
            Masukkan NISN Anda untuk check-in kehadiran.
          </CardDescription>
        </CardHeader>
        <CardContent className="p-8">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid gap-2">
              <Label htmlFor="nisn" className="text-gray-700 dark:text-gray-200 text-lg">Nomor Induk Siswa Nasional (NISN)</Label>
              <Input
                id="nisn"
                type="text"
